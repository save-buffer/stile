...
def test_basic_matmul(reset):
    ...
    a = tjax.random.normal(k1, M, N)
    b = tjax.random.normal(k2, N, K)
    c = tjax.TypedResult("(M N, N K -> M K)")
    ...
    for im in range(0, 10, 5):
        for ik in range(0, 20, 5):
            ...
            for in_ in range(0, 15, 5):
                ...
                tile_c = tjax.einsum(tile_a, tile_b, "M N, N K -> M K")
                c_accum = c_accum + tile_c
            assert isinstance(c_accum, tjax.TypedJaxArray)
            c.assign(c_accum)
...
def test_online_softmax(reset):
    ...
    m1 = x_block1.max(N)
    exp1 = tjax.exp(x_block1 - m1.repeat(N[0:4]))
    l1 = exp1.sum(N)

    x_block2 = x.slice(N, 4, 8)
    m2 = x_block2.max(N)
    exp2 = tjax.exp(x_block2 - m2.repeat(N[4:8]))
    l2 = exp2.sum(N)

    m_global = tjax.maximum(m1, m2)

    l1_correction = tjax.exp(m1 - m_global)
    l1_corrected = l1 * l1_correction

    l2_correction = tjax.exp(m2 - m_global)
    ...
    softmax2 = tjax.exp(x_block2 - m_global.repeat(N[4:8])) / l_global.repeat(N[4:8])
...
def test_flash_attention(reset):
    ...
    for iqctx in range(0, qctx.size, qctx_tile_size):
        ...
        for ictx in range(0, nctx.size, nctx_tile_size):
            ...
            qk_tile = tjax.einsum(q_tile, k_tile, "qctx dhead, nctx dhead -> nctx qctx") / jnp.sqrt(dhead.size)
            ...
            new_l = tjax.exp(running_max - new_max) * running_l + tjax.exp(tile_max - new_max) * tile_l
            
            v_tile = V.slice(nctx, ictx, ictx + nctx_tile_size)
            v_proj = tjax.einsum(logits, v_tile, "nctx qctx, nctx dhead -> qctx dhead")
            
            rescaled_old_o = (running_l * tjax.exp(running_max - new_max)).repeat(dhead).rearrange(qctx, dhead) * o
            rescaled_v_proj = tjax.exp(tile_max - new_max).repeat(dhead).rearrange(qctx, dhead) * v_proj
